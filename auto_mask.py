...
# def export_parts_as_scene(mesh, face_ids, out_path, color_map=None, base_name="part"):
#     """
#     根據每個 face 的 label，將 mesh 拆成多個 part，輸出成一個 glb scene。
#     - mesh: trimesh.Trimesh
#     - face_ids: (num_faces,) 每個 face 的 cluster/part id，-1/-2 代表無效或背景
#     - out_path: 要輸出的 .glb 路徑
#     - color_map: dict[label] = (r,g,b)，可選；如果有就幫每個 part 上一個顏色
#     """
#     face_ids = np.asarray(face_ids)
#     unique_ids = np.unique(face_ids)
#     scene = trimesh.Scene()
#     for lab in unique_ids:
#         # 跳過背景 / 無效 label
#         if lab < 0:
#             continue
#         # 1. 取出這個 label 對應的 faces
#         mask = (face_ids == lab)
#         faces_l = mesh.faces[mask]
#         if faces_l.shape[0] == 0:
#             continue
#         # 2. 找出這些 faces 用到的 vertex index
#         used_vert_idx = np.unique(faces_l.reshape(-1))
#         # 3. 建新的 vertex array
#         verts_l = mesh.vertices[used_vert_idx]
#         # 4. 把 faces remap 成 0 ~ n-1
#         index_map = -np.ones(mesh.vertices.shape[0], dtype=np.int64)
#         index_map[used_vert_idx] = np.arange(len(used_vert_idx))
#         faces_l_remap = index_map[faces_l]
#         # 5. 建 Trimesh
...
#             process=False
#         )
#         # 6. 如果有 color_map，就給整個 part 一個顏色（可選）
#         if color_map is not None and lab in color_map:
#             c = np.array(color_map[lab], dtype=np.uint8)
#             if c.shape[0] == 3:
#                 c = np.concatenate([c, np.array([255], dtype=np.uint8)])  # 加 alpha
#             face_colors = np.repeat(c.reshape(1, 4), faces_l_remap.shape[0], axis=0)
#             part_mesh.visual.face_colors = face_colors
#         # 7. 加到 scene 裡，一個 label 一個 node
#         scene.add_geometry(part_mesh, node_name=f"{base_name}_{int(lab)}")
#     scene.export(out_path)
#     print(f"[export parts scene] {out_path}")
import copy
def export_parts_as_scene(mesh, face_ids, out_path, base_name="part"):
    """
    根據每個 face 的 label，將 mesh 拆成多個 part，輸出成一個 glb scene，
    並保留原本的材質與貼圖（uv）。
    """
    ...
    for lab in unique_ids:
        ...
        if mesh.visual is not None:
            part_mesh.visual = copy.deepcopy(mesh.visual)
            if has_uv:
                # 只取對應頂點的 uv
                part_mesh.visual.uv = mesh.visual.uv[used_vert_idx]
        scene.add_geometry(part_mesh, node_name=f"{base_name}_{int(lab)}")
    scene.export(out_path)
    print(f"[export parts scene with texture] {out_path}")
...
def mesh_sam(
    ...
    with Timer("计算最后的aabb"):
        aabb = get_aabb_from_face_ids(mesh, final_face_ids)
    # ✅ 新增：根據 final_face_ids 直接輸出「每個 part 一個物件」的 glb
    if save_path is not None:
        parts_scene_path = os.path.join(save_path, "auto_mask_mesh_final_parts.glb")
        export_parts_as_scene(
            mesh,
            final_face_ids,
            parts_scene_path,
            # color_map=color_map_2,   # 保留顏色（可拿掉）
            base_name="part"
        )
    return aabb, final_face_ids, mesh
